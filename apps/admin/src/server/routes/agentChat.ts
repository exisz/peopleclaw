...
agentChatRouter.get('/apps/:appId/agent-sessions', requireAuth, requireTenant, async (req, res) => {
  ...
  if (!checked) return;
  res.json(listAgentSessions(checked.tenantId, checked.app.id));
});
...
agentChatRouter.post('/apps/:appId/agent-sessions', requireAuth, requireTenant, async (req, res) => {
  ...
  res.status(201).json(createAgentSession({ tenantId: checked.tenantId, appId: checked.app.id, title }));
});

agentChatRouter.get('/apps/:appId/agent-sessions/:sessionId', requireAuth, requireTenant, async (req, res) => {
  const checked = await requireTenantApp(req, res);
  if (!checked) return;
  const session = readAgentSession(checked.tenantId, checked.app.id, req.params.sessionId);
  if (!session) {
...
agentChatRouter.delete('/apps/:appId/agent-sessions/:sessionId', requireAuth, requireTenant, async (req, res) => {
  const checked = await requireTenantApp(req, res);
  if (!checked) return;
  deleteAgentSession(checked.tenantId, checked.app.id, req.params.sessionId);
  res.status(204).end();
});
...
agentChatRouter.post('/apps/:appId/agent-sessions/:sessionId/messages', requireAuth, requireTenant, async (req, res) => {
  ...
  appendAgentMessage(checked.tenantId, checked.app.id, session.id, { role: 'user', content: message });
  try {
    ...
    appendAgentMessage(checked.tenantId, checked.app.id, session.id, { role: 'assistant', content: assistantContent });
  } catch (e) {
