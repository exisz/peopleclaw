    ...
    <aside className="w-72 flex-shrink-0 flex flex-col overflow-hidden" style={{ background: '#0f0f23' }}>
      ...
      <div className="px-5 pt-4 pb-2">
        <input
          ...
          className="w-full px-3 py-2.5 rounded-lg text-xs bg-white/5 border border-white/5 text-gray-300 placeholder-gray-600 outline-none transition-all duration-200 focus:border-[#00d2ff]/30 focus:bg-white/[0.07] focus:shadow-[0_0_0_3px_rgba(0,210,255,0.08)]"
          style={{ fontFamily: 'DM Sans, sans-serif' }}
        />
      </div>

      {/* Workflow list */}
      <nav className="flex-1 overflow-y-auto py-2 px-5">
        {grouped.map(g => (
          <div key={g.category} className="mb-4">
            <p className="font-mono text-[10px] uppercase tracking-widest text-gray-500 mb-2 pl-1">{g.category}</p>
            {g.items.map(w => (
              <button
                key={w.id}
                onClick={() => onSelect(w)}
